...
public class FinancialAuditController extends BaseController{
     ...
     * @param req
     */
    @RequestMapping(value = "/getProjectFinancialAuditCount.htm", method = RequestMethod.POST)
    public void getProjectFinancialAuditCount(HttpServletResponse resp,AuditForm form,HttpServletRequest req){
        Map<String, Object> model = new HashMap<String, Object>();
        try {
            GardenProjectAuditQuery query = new GardenProjectAuditQuery();
            query.setId(form.getId());
            ...
            query.setUpdateUserId(SpringSecurityUtil.getIntPrincipal());
            query.setPageBean(new PageBean(Integer.MAX_VALUE, GenerateConstants.number_1));
            ResultEntry<Integer> result = gardenProjectAuditFacade.getProjectFinancialAuditCount(query);
            if (result.isSuccess()) {
