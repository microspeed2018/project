...
public class LeaderAuditController extends BaseController{
     ...
     * @param req
     */
    @RequestMapping(value = "/getProjectLeaderAuditCount.htm", method = RequestMethod.POST)
    public void getProjectLeaderAuditCount(HttpServletResponse resp,AuditForm form,HttpServletRequest req){
        Map<String, Object> model = new HashMap<String, Object>();
        try {
            GardenProjectAuditQuery query = new GardenProjectAuditQuery();
            query.setId(form.getId());
            ...
            query.setUpdateUserId(SpringSecurityUtil.getIntPrincipal());
            query.setPageBean(new PageBean(Integer.MAX_VALUE, GenerateConstants.number_1));
            ResultEntry<Integer> result = gardenProjectAuditFacade.getProjectLeaderAuditCount(query);
            if (result.isSuccess()) {
