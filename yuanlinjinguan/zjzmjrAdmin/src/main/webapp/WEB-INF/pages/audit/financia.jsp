...
<body>
    ...
    <script type="text/javascript">
        ...
        $(function() {
            $("#datagrid").datagrid(
                {
                    url : '${ctx}/financialAudit/user/getProjectFinancialAudit.htm',
                    title:'财务审核',
                            ...
                            formatter: function (value, row, index) {
                                if (!row.status) {
                                    if(row.type==26){
                                       ...
                                       return "<a href='javascript:result("+ index +", 1)'>通过</a>|<a href='javascript:result("+ index +", 2)'>不通过</a>";
                                    }
        ...
        function result(index, status) {
            ...
            if(status==2){
               $.messager.prompt('提示', '请输入不通过理由', function(r){
	              if (r){
	              		$.ajax({
                		url: "/financialAudit/user/updateChiefAuditById.htm",
		                type: 'post',
		                dataType: 'json',
		                data: {
		                    status : status,
