...
function fSetColor(){
	...
	if(dvForeColor.getElementsByTagName("TABLE").length == 0){
...
function setMode(bStatus){
	...
	if(bStatus){
		...
		if(gIsIE){
			HtmlEditor.style.height = "400px";
			divEditor.style.height = "400px";
		}else{
			HtmlEditor.style.height = "400px";
			divEditor.style.height = "400px";
		}
...
function createImg(){
	if(window.ActiveXObject){
		var sPhoto=window.showModalDialog("upload.html",window,"dialogWidth=500px;dialogHeight=200px;resizable=0;help=0;scroll=0;center=1;status=0;");
		setImgInfo(sPhoto);
	}else{
		window.open("upload.html",'',"height=200, width=500, toolbar=no, menubar=no, scrollbars=no, resizable=no,location=no, status=no'");
	}
}
function setImgInfo(sPhoto){
	console.log(sPhoto);
