...
$(function(){
	...
	$(document).ajaxError(function(event,request, settings){
		 ...
		 $.messager.alert('Warning','服务器响应错误');
		 checkAuth();
	});
	...
	$(".query input[type!=button]").keyup(function(e){
		if(e.keyCode==13){
			$("input[type='button'][value='搜索']").click();
		}
	});
...
function fixToolbarPos(){
	$(".win-toolbar").width($(window).innerWidth());
	$(".win-toolbar").css("top",$(window).innerHeight()-38).css("display","block");
}

function openWindow(url,title,param,width,height,ps){
	var zindex = _zindex++;
	var wid = "_iframe_window_"+zindex;
	var ps = getRequestParam(param);
	var size = $.init(ps, getProperSize(width,height));
	if (!width || !height) {
		width = getProperSize(width,height).width;
		height = getProperSize(width,height).height;
	}
	ps = ''==$.trim(ps)?'?wid='+wid:'?'+ps+'&wid='+wid;
	$("<div id='"+wid+"' name='popWin'><iframe src='' style='border:0;width:100%;height:100%'></iframe></div>").appendTo($("body"));
	$("#"+wid).window({
		fit:false,
		collapsible:false,
		minimizable:false,
		...
		title:title?$.trim(title):'',
	 	onClose:function(){$("#"+wid).window("destroy");}
	});
	$("#"+wid+" iframe").on("load",function(){
		$("body",$($("#"+wid+" iframe").focus().get(0).contentWindow.document)).keyup(function(e){
...
function getProperSize(width,height){
	...
	if(!isEmpty(width) && width == 100){
		size.width = $(document).innerWidth();
	}else if(!isEmpty(width)){
		size.width = ($(document).innerWidth()-180)*width/100;
	}else{
		size.width = $(document).innerWidth()-180;
	}
	if(!isEmpty(height) && height == 100){
		size.height = $(document).innerHeight();
	}else if(!isEmpty(height)){
		size.height = ($(document).innerHeight()-60)*height/100;
	}else{
		size.height = $(document).innerHeight()-60;
	}
	return size;
}
...
var X={};
X.address={
	p:null, c:null, dp:'', dc:'',nosel:false,
	...
	loadCity:function(pid){
		...
		t.c.empty();
		if(pid==0){t.addOption(t.c,'选择市','0');return;}
