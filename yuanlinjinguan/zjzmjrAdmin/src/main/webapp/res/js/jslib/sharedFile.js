		...
		buttons : [ {
			...
			handler : function() {
				...
				if(fileName == "" && fileName == null){
					$.messager.alert("提示", "请输入资料名称", "info");
					return;
				}
...
//				}else{
//				    $("#uploadCommonForm").attr("action", "/sharedFile/user/fileUpLoad.htm");
//				}
				$("#uploadCommonForm").attr("action", "/sharedFile/user/fileUpLoad.htm");
...
//		return;
//	}
    window.location.href = "/sharedFile/user/downloadFiles.htm?saveAddress="+fileAddress;
	 /*$.ajax({
...
function deleteFileUpload(fileId){
    $.messager.confirm("确认", "确认要删除吗？删除后无法恢复。", function (r) {
          if (r) {
        	  ...
        	  $.ajax({
      			url : "/sharedFile/user/deleteFileManageById.htm",
      			type : 'post',
      			dataType : 'json',
      			data : {
      				id : fileId
